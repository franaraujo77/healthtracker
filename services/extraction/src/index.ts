...
await boss.work<JobPayload<unknown>>(
  'extraction.dead_letter',
  async (jobs) => {
    for (const job of jobs) {
      try {
        ...
        if (!correlationId) {
          console.warn(`[pg-boss] dead-letter: job ${job.id} has no correlationId — skipping markUploadFailed`)
          continue
        }
        ...
        if (typeof job.name === 'string' && job.name.startsWith('extraction.')) {
          ...
          console.warn(`[pg-boss] dead-letter: job ${job.id} (${job.name}) is not an extraction job — skipping markUploadFailed`)
        }
      } catch (err) {
        console.error(`[pg-boss] dead-letter: error processing job ${job.id}`, err)
      }
    }
  },
)
