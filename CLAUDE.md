...
- **Deferred to later Epic 9 stories:** the `consumers/document.ts` `extract()` failure-path catch/dead-letter (9.3); stub-era `failed` re-enqueue (9.4). Sync `AnalyzeDocument` `Bytes` is single-page / ≤10 MB; multi-page async `StartDocumentAnalysis` is a future story.
