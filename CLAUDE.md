...
- **Story 7.6 (Epic 7 batched migration) — SHIPPED.** `supabase/migrations/0009_epic_7_personal_context.sql` consolidates 6 enums (`life_event_category_enum`, `life_event_privacy_flag_enum`, `emotional_checkin_state_enum`, `emotional_checkin_type_enum`, `emotional_checkin_privacy_enum`, `voice_memo_privacy_enum`), 3 tables (`life_events`, `emotional_checkins`, `voice_memos`), `ALTER TABLE uploads ADD COLUMN viewed_at TIMESTAMPTZ` (additive, nullable, no backfill), 5 indexes (incl. `(upload_id, type)` UNIQUE for emotional*checkins and `(upload_id)` UNIQUE for voice_memos), 2 CHECK constraints, 6 patient RLS policies (verbatim mirror of `custom_rls*\*.sql`files), the private`voice_memos`Storage bucket, and 2 Storage RLS policies scoped via`storage.foldername(name)[1]`. **All indexes are non-partial** → ships in the standard `supabase/migrations/`path; NO`migrations-postapply/` companion needed.
- **Deferred for post-Epic-7 cleanup:** the AC10 privacy-enum unification (`ALTER TYPE life_event_privacy_flag_enum RENAME TO personal_context_privacy_enum` + collapse `emotional_checkin_privacy_enum` + `voice_memo_privacy_enum` into the unified one + update Drizzle source). Story 7.6 deliberately did NOT do the rename because it would couple the batched-migration delivery to a Drizzle source refactor + integration-test revalidation that this story should not own. A dedicated cleanup story can perform the rename atomically once Epic 7 ships.
