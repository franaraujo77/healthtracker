...
| `EXTRACTION_ADAPTER`    | server | no                    | `mock`      | Textract adapter selection: `mock` (fixture-driven, CI/dev — NFR-S8 forbids live AWS in CI) or `aws` (production). Unknown values fail loud at boot.                                                                                                                        | Story 2.3  |
| `AWS_REGION`            | server | when `aws`            | `sa-east-1` | Pinned to `sa-east-1` for patient-data residency (NFR-S8). Any other value **fails the boot gate**.                                                                                                                                                                         | Story 9.2  |
| `AWS_ACCESS_KEY_ID`     | server | when `aws`            | —           | AWS access key for Textract. **Must reference the signed-DPA AWS account** (LGPD Art. 33). Required at boot unless a container/web-identity task role supplies credentials.                                                                                                 | Story 9.2  |
| `AWS_SECRET_ACCESS_KEY` | server | when `aws`            | —           | AWS secret key paired with `AWS_ACCESS_KEY_ID`. From the signed-DPA account. Never expose client-side.                                                                                                                                                                      | Story 9.2  |
...
### AWS Textract DPA + region gating (Story 9.2)

When `EXTRACTION_ADAPTER=aws`, the extraction worker processes patient lab documents through AWS Textract. Two LGPD/NFR constraints are enforced at **worker boot** (`assertAwsTextractConfig`, `services/extraction/src/textract/aws-config.ts`) so a misconfigured deploy crashes loud instead of silently dead-lettering every upload:

- **Region pin (NFR-S8 data residency):** `AWS_REGION` must be `sa-east-1` (or unset, which defaults to it). Any other value fails the boot gate — patient documents must not leave the region.
- **DPA-signed credentials (LGPD Art. 33):** Textract may only run against an AWS account with a **signed Data Processing Agreement**. The credentials (`AWS_ACCESS_KEY_ID` + `AWS_SECRET_ACCESS_KEY`, or a container/web-identity task role) must reference that account. The boot gate verifies credentials are _present_ (not that they are _valid_ — a live STS/Textract probe would violate NFR-S8's no-live-AWS-in-CI rule and couple startup to AWS uptime); invalid/expired creds surface at first dispatch. Credential sources without an env footprint — `AWS_PROFILE`/shared-config (`~/.aws`), SSO, EC2-IMDS — are intentionally not accepted (the Railway deploy target injects static keys); running the worker elsewhere requires static keys, a container role, or a full web-identity (`AWS_WEB_IDENTITY_TOKEN_FILE` + `AWS_ROLE_ARN`).
