...
3-layer adversarial round-2 (Blind Hunter + Edge Case Hunter + Acceptance Auditor) on the patched code. **2 HIGH (round-1 AC-regression + TOCTOU race) + 4 Med + 2 Low.** 8 patches applied (R2-P171–R2-P178), 5 deferred (F143–F147), 8 dismissed.

**`patch` (must fix before done):**

- [x] [Review][Patch] **R2-P171 [HIGH AC-regression]: `postOnboardingImportRoute("photo")` query param was never consumed** [`apps/web/src/app/inicio/page.tsx` + `inicio-empty-state.tsx`, `apps/expo/src/app/(tabs)/inicio.tsx`] — R1-P153 routed failed-card "Enviar uma foto" to `/inicio?source=post_onboarding_photo` but the Início pages didn't read the param. AC4's three recovery options collapsed to two effective behaviors. Fix: parse `searchParams` on the web page, parse `useLocalSearchParams` on Expo; pass through as `initialSource` prop / local state to auto-open the source picker on `post_onboarding_photo`.
- [x] [Review][Patch] **R2-P172 [HIGH data-integrity]: TOCTOU race on `notification.upload_failed` / `complete` audit dedup** [`packages/db/src/schema/audit.ts`, `services/extraction/src/notifications/emit.ts`, `packages/api/src/uploads-review.ts`] — R1-P152's `SELECT EXISTS … audit_log` then INSERT inside `sql.begin` ran at READ COMMITTED; two concurrent transactions could both observe "no row" and both INSERT. Fix: partial unique index on `audit_log(resource_id, event) WHERE event IN ('notification.upload_complete', 'notification.upload_pending_review', 'notification.upload_failed')` + `ON CONFLICT ON CONSTRAINT audit_log_notification_event_unique DO NOTHING` in `emit.ts`'s raw INSERT (and a `try/catch` for SQLSTATE 23505 in `uploads-review.ts`'s `writeAuditLog` call). The second writer is silently skipped, including its pg-boss enqueue.
- [x] [Review][Patch] **R2-P173 [Med correctness]: Expo Push batch chunking can produce duplicate pushes on mid-chunk retry** [`services/extraction/src/consumers/notifications.ts`] — When chunk N+1 throws, pg-boss retries the whole job and chunk N resends. Fix (minimum): include chunk index + already-delivered count in the thrown error message so ops can correlate dupes. Full chunk-state persistence deferred to F143.
- [x] [Review][Patch] **R2-P174 [Med correctness]: `ExpoPushTokenSchema` accepted `ExponentPushToken[]` and 100KB strings** [`packages/api/src/router/notifications.ts`] — `startsWith`/`endsWith` regex was too loose. Fix: tighten to `/^ExponentPushToken\[[A-Za-z0-9_-]{18,40}\]$/` and add `.max(64)` length cap.
- [x] [Review][Patch] **R2-P175 [Med correctness]: `UPLOAD_STATUS_LABELS_PT_BR[row.status]` would render `"undefined"` for unknown statuses** [`apps/expo/src/app/(tabs)/historico.tsx`, `apps/web/src/app/inicio/historico/historico-client.tsx`] — Fix: cast through `Record<string, string | undefined>` to widen the index type and provide a fallback to the raw status string + neutral badge class.
- [x] [Review][Patch] **R2-P176 [Med a11y]: Two buttons labeled "Pular este resultado" on each failed Expo card** [`apps/expo/src/app/(tabs)/historico.tsx`] — The inner Card's `Button` was dead code (no onPress). VoiceOver/TalkBack would announce two identical buttons; the first non-functional. Fix: delete the inner Button; only the outer dismiss button remains.
- [x] [Review][Patch] **R2-P177 [Low hygiene]: `INVALID_CURSOR` guard is technically unreachable after Zod ISO validation** — Documented in R1-P159 as defense-in-depth; left in place. No code change.
- [x] [Review][Patch] **R2-P178 [Low coverage]: Handler `registerNotificationsConsumer` had zero unit-test coverage** [`services/extraction/__tests__/notifications.test.ts`] — Added three handler-level tests covering (a) zero-tokens skip, (b) `DeviceNotRegistered` token revoke, (c) missing upload row skip. Fake `boss.work` captures the handler; scripted sql + fake `ExpoPushClient`.

**`defer` (added to deferred-work.md):**

- [x] [Review][Defer] **F143** Per-chunk receipt persistence so mid-chunk retry doesn't resend earlier chunks. Pairs with F136.
- [x] [Review][Defer] **F144** Refactor: have `writeAuditLog` natively support `ON CONFLICT DO NOTHING` for notification events instead of catching SQLSTATE 23505 at the call site.
- [x] [Review][Defer] **F145** Prometheus counter `notification_send_skipped_total{reason="no_tokens"}` for ops visibility.
- [x] [Review][Defer] **F146** Vocabulary-rename consumer audit — R1-P161 renamed `empty_extraction` → `no_readable_text`. Sweep future analytics consumers.
- [x] [Review][Defer] **F147** Persisted dismiss state — Histórico local `dismissed` Set evaporates on tab remount; confirm with PM whether tab-scoped is acceptable or whether it should persist (per-patient kv).

**Dismissed (~8):** R1-P153 superseded by R2-P171; R1-P156 lab_name subquery (per-notification, not per-row); R1-P162 static import (confirmed safe); R1-P163 mutual-exclusion comment (race captured in R2-P172); R1-P164 a11y on Card (confirmed); R1-P157 chunking happy path (regression captured in R2-P173); R1-P160 empty-state (correct; "Mostrar pulados" deferred to F147); F135 Expo client hook (explicitly out of scope).

### Change Log

- 2026-05-22 — Code review round 2. **8 patches applied (R2-P171–R2-P178), 5 deferred (F143–F147), ~8 dismissed.** Two HIGH fixes closed: **R2-P171** the `?source=` query param is now read by both the web and Expo Início pages and wired through to auto-open the source picker when `source=post_onboarding_photo` — AC4's photo recovery option now works end-to-end; **R2-P172** added a partial unique index `audit_log_notification_event_unique` on `(resource_id, event)` for the three notification events + `ON CONFLICT ON CONSTRAINT … DO NOTHING` in the worker's raw INSERT and a 23505 catch in the API path — the TOCTOU race that R1-P152's read-then-write check couldn't close is now DB-enforced. Med fixes: R2-P173 logs chunk progress in the thrown error message; R2-P174 tightens the Expo push-token regex + length cap; R2-P175 casts the status-label indexed access through `Record<string, string | undefined>` so a future enum extension can't render "undefined" in the UI; R2-P176 deletes a dead duplicate "Pular este resultado" button inside the Expo Card. Low: R2-P178 adds three handler-level tests for the notifications consumer (zero-tokens, DeviceNotRegistered revoke, missing upload). **171 unit tests green** (+3 over round-1). Typecheck, lint, format all green.
