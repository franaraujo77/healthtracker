...
3-layer adversarial round-2 (Blind Hunter + Edge Case Hunter + Acceptance Auditor) on the patched code. 9 patches to apply, 7 deferred, ~10 dismissed.

**`patch` (must fix before done):**

- [x] [Review][Patch] **R2-P84: `pickImages` bare `catch` (P77) swallows the underlying error — no telemetry, no breadcrumb** [`apps/expo/src/hooks/use-import-files.ts`] — production launch failures will be silently invisible. Fix: `catch (err) { console.warn("[pickImages] launch error", err); return { ... } }` matching the P80 pattern.
- [x] [Review][Patch] **R2-P85: Início handlers `console.warn` on every permission deny — including the expected first-time "I want to think about it" tap — pollutes Sentry/logs** [`apps/expo/src/app/(tabs)/inicio.tsx:handlePickImageLibrary, handlePickImageCamera`] — distinguish a `permission-denied` rejection (expected user choice; don't log) from a `launch-error` rejection (unexpected; log). Fix: filter the rejected list by `uri !== "permission-denied"` before warning, or branch the log severity.
- [x] [Review][Patch] **R2-P86: P82's fallback string "Tente novamente em alguns instantes." is hardcoded in `extraction-pulse.tsx` instead of `packages/validators/src/index.ts`** [`packages/ui/src/extraction-pulse.tsx`] — every other pt-BR string in the file is sourced from validators (Story 1.5 / 2.1 / 2.2 Task 8 invariant). Fix: extract as `UPLOAD_FAILED_GENERIC_RETRY_PT_BR` in validators; import in the component.
- [x] [Review][Patch] **R2-P87: `requestCameraPermissionsAsync` / `requestMediaLibraryPermissionsAsync` themselves can throw** [`apps/expo/src/hooks/use-import-files.ts:pickImages`] — not wrapped in the P77 try/catch (only the launches are). Hardware-unavailable / OS-revocation-race throws bubble unhandled. Fix: extend the try/catch to wrap the permission request.
- [x] [Review][Patch] **R2-P88: P79 sentinel `fileSize ?? 1` only applied to camera path — library pick with missing `fileSize` on Android still rejects as `UPLOAD_EMPTY_FILE_PT_BR`** [`apps/expo/src/hooks/use-import-files.ts`] — library captures can have null `fileSize` on the same Android OEMs that drop it for camera. Fix: drop the `isCamera ?` constraint; apply the sentinel for both library and camera (server's P51 cap covers both via the same `stored.sizeBytes > UPLOAD_MAX_BYTES` check at confirmImport line 113 — verified to run for ALL mime types, not PDF-only).
- [x] [Review][Patch] **R2-P89: Web `<input accept>` attributes missing `image/heif`** [`apps/web/src/app/inicio/inicio-empty-state.tsx`] — P81 widened the validators allowlist + ExtractDocumentPayload.mimeType to include `image/heif`, but the web `<input>` `accept="image/jpeg,image/png,image/heic"` (library + camera inputs) doesn't. iPhone HEIC photos labeled `image/heif` get greyed out by the OS picker before reaching the server widening. Fix: append `,image/heif` to both `accept` attributes.
- [x] [Review][Patch] **R2-P90: Fixed rejection URIs (`permission-denied`, `camera-launch-error`, `library-launch-error`) collide on second occurrence — React key warnings, second row swallowed** [`apps/expo/src/hooks/use-import-files.ts`] — onboarding maps `rejected.map((file) => <Text key={file.uri} ...>)` (line 201 in import.tsx). Two consecutive denials collide. Fix: append `Date.now()` (or `crypto.randomUUID()` — Story 2.1 R2-P66 pattern) to each fixed URI.
- [x] [Review][Patch] **R2-P91: Onboarding `handlePick` and `handlePickCamera` share `isPickingRef` but neither button visually disables while the sibling is open — patient taps get silently dropped** [`apps/expo/src/app/onboarding/import.tsx`] — Início pairs the guard with `pdfDisabled` / `photoDisabled` visual state. Onboarding doesn't. Fix: track a `useState<boolean>(false)` `isPicking` mirror; pass `disabled={isPicking || isUploading}` to both Buttons.
- [x] [Review][Patch] **R2-P92: `packages/types/src/jobs.ts` modified by P81 but missing from the File List + got an incidental whole-file semicolon reformat** [`packages/types/src/jobs.ts` + story spec File List] — File List drift (CLAUDE.md §3 "surgical changes" violation). Fix: revert the whole-file reformat to leave only the union-widening lines changed; add `packages/types/src/jobs.ts` to the File List.

**`defer` (added to `deferred-work.md`):**

- [x] [Review][Defer] F96: ExtractionPulse `failed`-state fallback only renders when ALL three recovery callbacks are undefined — caller with only `onRetake` set sees one button + no fallback. Decide policy: caller-controlled, or always render fallback alongside?
- [x] [Review][Defer] F97: P78 removed the `result.assets ?? []` defensive guard for lint cleanliness — trades runtime safety for lint score. Restore with `eslint-disable-next-line` if a future Expo upgrade returns `undefined` instead of empty array.
- [x] [Review][Defer] F98: Web `validateClientSide` may receive empty `file.type` for HEIC on older Safari — `isUploadMimeType('')` returns false → user sees "unsupported mime" for a valid HEIC. Pre-existing issue, exposed more after P81 widening.
- [x] [Review][Defer] F99: Filename without extension (`"photo"`, `"IMG_001"`) reaches the synthetic-name fallback `image-${Date.now()}.jpg` and the rejection row loses the original label — patient can't identify which capture failed.
- [x] [Review][Defer] F100: Web `openPicker` `setTimeout(reset, 250)` could race the `change` handler on slow devices (releases `isPickingRef` before the upload starts → double-tap spawns second picker mid-upload). Stale `cancel` listener can also fire on next picker open.
- [x] [Review][Defer] F101: Two camera captures in one session sharing the same `asset.uri` (rare on Android cache reuse) collide in `progressByPath[uri]`. Use synthetic per-pick id when keying.
- [x] [Review][Defer] F102: Permission revoked mid-flow surfaces as `GENERIC_UPLOAD_ERROR_MESSAGE_PT_BR` — no hint to re-grant in Settings. Distinct copy + deep-link to Settings would help.

**Dismissed (~10):** Blind High #1 P79 server-bypass (verified — `confirmImport:113` re-stats for ALL mime types, not PDF-only; orphan storage covered by F49); Blind Med P81 server allowlist incomplete (server schema uses `z.enum(UPLOAD_ALLOWED_MIME_TYPES)` — widening propagates automatically); Blind Med P75 iOS camera HEIC regression (iOS camera DOES populate `asset.mimeType` per expo docs; verified safe by Auditor); Blind Low P81 worker mime widening (Edge verified worker imports shared type, no narrow override); Blind Low `cameraHintIsWeb` leaky abstraction (cosmetic — two strings is fine); Edge #6 Início rejected-only no patient surface (already F95 from R1); Auditor F-R2-3 P79 client-side bypass (server re-validates; acceptable for v1); Auditor F-R2-4 through F-R2-8 (verification confirmations, not findings).

---

**Dismissed (R1, 16):** B2 web allowlist widening (server's storage-reported content-type check already enforces — Story 1.5 P49); B3 `handleFileInput` shared across inputs (per-channel `accept` is UX hint, not security; validateClientSide gates server-side); B4 sheet stays open during upload (matches Story 2.1 R2-P69 established pattern, not a deviation); B8 ExtractionPulse opacity in failed state (`usePulseOpacity` correctly returns 1 when `!active`); B9 camera input lacks `multiple` (by design — single-capture); B10 grep for deleted-constant consumers (typecheck passes); B11 useCallback empty deps fine; B12 `photoDisabled` = `pdfDisabled` (single-batch upload intentional this story); E1 runtime source validation (TS prevents); E9 Tamagui Button disabled web (F79 already deferred); E10 web cancel/focus reset (already implemented by Story 2.1 R2-P65, generalized through `openPicker`); E11 empty `file.type` case (`isUploadMimeType('')` returns false → rejected as expected); E14 desktop user tapping camera shows file picker (intended UX with hint copy); E16 stale callbacks risk (folded into F94); E17/E18 sheet rendering with various callback combos (independent props, intended); E20 launch cancelled (handled at L467); E21 shared isPickingRef blocks cross-source (intentional one-picker-at-a-time); E23 openPicker null ref (handled); A2 deleted-vs-deprecated constants (documented Completion Note #5); A3 Clarification #4 deviation (documented).
...
- 2026-05-22 — Code review round 2. 9 patches resolved (R2-P84–R2-P92), 7 deferred (F96–F102), 10 dismissed (notable: Blind High "P79 server-side bypass" was a false positive — `confirmImport:113` re-stats `stored.sizeBytes > UPLOAD_MAX_BYTES` for ALL mime types, not PDF-only; storage orphan covered by F49). **Notable fixes**: R2-P84 (the round-1 `catch {}` bare swallow → `catch (err)` + `console.warn` for telemetry visibility); R2-P85 (Início handlers no longer warn on permission denials — `unexpectedRejections` filter — to avoid polluting Sentry on every "I want to think about it" tap); R2-P86 (P82's "Tente novamente em alguns instantes." fallback extracted to `UPLOAD_FAILED_GENERIC_RETRY_PT_BR` in validators — Story 2.1 / 2.2 invariant that pt-BR copy lives in validators); R2-P87 (wrapped `requestCameraPermissionsAsync` / `requestMediaLibraryPermissionsAsync` in try/catch — the permission requests themselves can throw on hardware unavailable / OS revocation race); R2-P88 (P79's `fileSize ?? 1` sentinel now fires for library picks too, not just camera — Android library picks with missing fileSize were silently rejected as `UPLOAD_EMPTY_FILE_PT_BR`); R2-P89 (added `image/heif` to all three web `<input accept>` attributes — P81 widened the server-side allowlist but the OS picker was still filtering legit iPhone HEIC out); R2-P90 (unique URI per rejection — `permission-denied-${Date.now()}`, `camera-launch-error-${Date.now()}` etc. — fixed URIs collided on second occurrence and caused React key warnings + lost rows); R2-P91 (onboarding `handlePick` / `handlePickCamera` now use a `useState` mirror of `isPickingRef` so sibling buttons visually disable while a picker is open); R2-P92 (`packages/types/src/jobs.ts` reformat reverted — the diff is now just the 4-line `image/heif` widening, matching CLAUDE.md §3 surgical-changes rule — and the file is added to the File List). 85 unit tests still green; lint, typecheck, format, test all green.
