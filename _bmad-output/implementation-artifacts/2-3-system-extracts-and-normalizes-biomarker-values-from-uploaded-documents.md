...
- Confidence gate (per-field 0.85 threshold; <0.01 dead-letter via `applyDeadLetter`). **R1-P100 / R2-P128 SUPERSEDED**: per-field dead-letter removed; every non-publishable field routes to review queue. Dead-letter only on empty extraction or zero-work outcome.
...
7. **Dead-letter trigger threshold**: AC says `confidence < 0.01` for individual fields. But what if 50% of fields are below 0.01? Currently spec says "ALL fields < 0.01 → dead-letter; ANY field >= 0.01 → publish-or-review the rest". **Recommended: ship as drafted.** A mostly-failed document with one high-confidence field is still useful. **R1-P100 / R2-P128 SUPERSEDED**: this contract was revised in R1 — every non-publishable field (including `confidence < 0.01`) now routes to `extraction_review_queue` with `reason='low_confidence'`. Dead-letter triggers only when `fields.length === 0` OR the dispatcher returns zero work of any kind. The "ALL fields < 0.01 → dead-letter" semantic no longer exists.
...
3-layer adversarial round-2 (Blind Hunter + Edge Case Hunter + Acceptance Auditor) on the patched code. **3 HIGH (all related to crash-recovery + transaction-boundary correctness)**, plus 14 Med/Low. 17 patches to apply, 8 deferred, ~5 dismissed.

**`patch` (must fix before done):**

- [x] [Review][Patch] **R2-P113 [HIGH B1/B3/E04/F-R2-02]: Terminal `applyUploadTransition` outside `sql.begin()` + review-queue has no idempotency seam** [`services/extraction/src/consumers/document.ts`] — R1-P109 wraps dispatch+audit in a transaction, but the terminal status UPDATE runs separately. A crash between begin-commit and the terminal UPDATE leaves observations + audit committed but status stuck in `processing`. R1-P95's resume then re-runs the dispatcher, which: (a) hits ON CONFLICT on observations (idempotent — good), (b) duplicates `extraction_review_queue` rows (NO idempotency seam — bad), (c) audit emit for the new dispatch returns empty `publishedObservationIds` so no fresh audit rows fire (but the original audit rows from the first attempt remain — combined with R2-P115 below). Fix: (i) include the terminal `applyUploadTransition` inside the `sql.begin()` block; (ii) add a unique index on `extraction_review_queue(upload_id, biomarker_name, reason)` so retries are idempotent.
- [x] [Review][Patch] **R2-P114 [HIGH B2]: `currentStatus()` mishandles `pending_review`** [`services/extraction/src/consumers/document.ts:75-95`] — current logic: `if processing → resume; else → ack-skip`. `pending_review` is non-terminal but falls into the skip branch. If a previous run wrote partial review-queue rows and moved to `pending_review` before crashing on a later UPDATE, the retry orphans the remaining fields. Fix: explicit enumeration: `processing → resume`; `pending_review|complete|failed → ack-skip with reason`; `queued|null → throw to retry`.
- [x] [Review][Patch] **R2-P115 [HIGH E-CR-21/22]: Idempotent retry of an already-complete upload dead-letters it** [`services/extraction/src/consumers/document.ts`] — re-processing where every observation ON-CONFLICT no-ops + every review-queue insert is a dup → `publishedCount === 0 && reviewQueueCount === 0` after the (now-idempotent) tx. Consumer routes to `applyDeadLetter`, which flips `processing → failed`. Fix: distinguish "no rows written because already-exist" from "no rows because empty" via a `conflictCount` counter returned from dispatch; dead-letter ONLY when `fields.length === 0` OR (`publishedCount === 0 && reviewQueueCount === 0 && conflictCount === 0`).
- [x] [Review][Patch] **R2-P116 [Med B5]: `downloadStorageObject` returns dead `mimeType`** [`services/extraction/src/index.ts:downloadStorageObject`] — R1-P98 documented dropping the storage-derived mimeType, but the function still returns it. Consumer destructures only `{ bytes }`. Fix: drop `mimeType` from the return type to match actual usage.
- [x] [Review][Patch] **R2-P117 [Med B6]: Misleading comment on numeric stringification + precision** [`services/extraction/src/pipeline/dispatch.ts`] — `String(0.1+0.2)` yields `'0.30000000000000004'`; stringification doesn't fix binary-float artifacts. R1-P101's comment claims it "avoids precision artifacts at 0.85-class boundaries" — it doesn't. Fix: update the comment to be accurate ("matches the API helper's serialization for consistency") rather than overpromising.
- [x] [Review][Patch] **R2-P118 [Med B7]: `resolveLoincCode` queried even for low-confidence fields** [`services/extraction/src/pipeline/dispatch.ts`] — every field hits the LOINC SELECT before the gate decides; low-confidence fields don't need the LOINC code. Fix: short-circuit — only call `resolveLoincCode` when `confidence >= 0.85`.
- [x] [Review][Patch] **R2-P119 [Med B8]: R1-P99 silently coerces invalid confidence to 0** [`services/extraction/src/pipeline/dispatch.ts`] — operator viewing the review queue sees `confidence_score=0.00` for fields whose adapter output was actually `NaN` / `1.5` / `-0.1`; can't distinguish "extractor said 0" from "extractor said garbage". Fix: write a `metadata: { raw_confidence: '<original>', invalid: true }` jsonb column on the review row, OR add a `confidence_invalid boolean` column to `extraction_review_queue`.
- [x] [Review][Patch] **R2-P120 [Med B9]: R1-P96 boot warning is misleading about dead-letter path** [`services/extraction/src/index.ts`] — message says "every job will dead-letter"; actually jobs throw from `extract()`, pg-boss retries+dead-letters the JOB (not the upload), then the `extraction.dead_letter` handler calls `markUploadFailed`. Eventually correct, but multi-hop. Fix: revise the message to "every job will fail with NO_FIXTURE and eventually mark the upload as failed via the dead-letter handler".
- [x] [Review][Patch] **R2-P121 [Med E-CR-05]: Single per-field exception bubbles → infinite retry loop** [`services/extraction/src/pipeline/dispatch.ts`] — a LOINC SELECT failure mid-batch rolls back the entire transaction; pg-boss retries; same bug → infinite loop. Fix: per-field try/catch; on error, route the field to review queue with `reason='low_confidence'` + error metadata, continue with siblings.
- [x] [Review][Patch] **R2-P122 [Med E-CR-06/07/24]: Empty biomarkerName or undefined valueText violates NOT NULL → batch rollback** [`services/extraction/src/pipeline/dispatch.ts`] — empty biomarker name routes to review queue insert which fails on `biomarker_name NOT NULL`; same for missing `valueText` → `value_text NOT NULL`. Fix: validate at entry — empty/missing → silently skip the field (NOT publish, NOT review); log the count.
- [x] [Review][Patch] **R2-P123 [Med E-CR-12]: `downloadStorageObject` permanent failure → infinite resume loop** [`services/extraction/src/consumers/document.ts`] — if Supabase storage permanently 404s the path, every retry re-enters the resume branch (status=processing), downloads fails, throws, pg-boss retries forever until retry limit hits the dead-letter handler. Cleaner: catch download failures and `applyDeadLetter` immediately with reason `storage_unavailable`. Fix: wrap `downloadStorageObject` call in try/catch; on error, `applyDeadLetter` + return.
- [x] [Review][Patch] **R2-P124 [Med E-CR-16]: `patientId` undefined at consumer entry → `::uuid` cast throws inside transaction** [`services/extraction/src/consumers/document.ts`] — Fix: validate `data.patientId` + `data.payload.uploadId` at handler entry; throw a clear error before any state change.
- [x] [Review][Patch] **R2-P125 [Med E-CR-20]: `writeReviewQueueEntry` lacks the finite-numeric validation R1-P108 added to `writeObservation`** [`packages/api/src/extraction-review.ts`] — NaN `confidenceScore` becomes `'NaN'` string, postgres numeric rejects at DB layer rather than helper layer. Fix: mirror R1-P108's `Number.isFinite` check.
- [x] [Review][Patch] **R2-P126 [Low B11]: `OBSERVATIONS_INSERT_SQL_SHAPE` is unused dead string** [`services/extraction/src/pipeline/dispatch.ts`] — exported "for the snapshot sync test" but no test imports it. Fix: delete.
- [x] [Review][Patch] **R2-P127 [Low B13]: `extraction-review.ts` comment claims operator UI "deduplicates by (upload_id, biomarker_name)"** [`packages/api/src/extraction-review.ts`] — but there's no unique index; UI dedup is the only line of defense. R2-P113 adds a unique index — update this comment to reflect that, OR remove the misleading claim.
- [x] [Review][Patch] **R2-P128 [Low F-R2-03]: Spec text + R1-P100 are internally inconsistent** [`_bmad-output/implementation-artifacts/2-3-...md` Clarification #7, Task 6, Completion Notes #7] — all describe the old "ALL fields < 0.01 → dead-letter" contract; R1-P100 replaced it. Fix: append a note to each section acknowledging the R1-P100 supersession ("dead-letter triggers only on empty extraction; per-field <0.01 routes to review queue as low_confidence").
- [x] [Review][Patch] **R2-P129 [Low E-CR-18 / F-R2-04]: Snapshot-sync test only imports worker's `UPLOAD_TRANSITIONS`** [`services/extraction/__tests__/snapshot-sync.test.ts`] — R1-P94's deviation rationale leans on this test as the drift guard; the test pins the worker's shape only. Fix: import the API helper's `UPLOAD_TRANSITIONS` too and assert `deepEqual`.

**`defer` (added to `deferred-work.md`):**

- [x] [Review][Defer] F115: `sql.begin` mock test (in `document-consumer.test.ts`) sets `tx === sql` so the production behavior of `postgres.js`'s real `TransactionSql` (forbids nested `.begin` without `.savepoint`) is untested. Add typing-only documentation; integration test deferred (F112 family).
- [x] [Review][Defer] F116: `awsTextractAdapter` import is eager in `index.ts` regardless of `EXTRACTION_ADAPTER`. Currently side-effect-free; when the real SDK ships, pulls `@aws-sdk/client-textract` into the mock-mode bundle. Switch to dynamic `await import()` then.
- [x] [Review][Defer] F117: `parseCollectedAt` ISO regex still rejects `"2024-03-15T00:00:00Z"` even after R1-P103 (R1-P103 strips space-then-time, not T-suffix). Extends F109.
- [x] [Review][Defer] F118: Worker `applyUploadTransition` raw SQL drift from API helper — `COALESCE(processing_started_at, now())` etc. may diverge from the API's Drizzle UPDATE; the snapshot test only pins the arc map, not the SQL body. Integration test required (extends F112).
- [x] [Review][Defer] F119: `pending_review → processing` is not in `UPLOAD_TRANSITIONS`; Story 8.2 reviewer-reject-retry can't be modeled in the current state machine. Design decision deferred.
- [x] [Review][Defer] F120: R1-P93 audit emission uses `actor_id = patientId` for system events; the actor is the worker, not the patient. Story 1.1 F10 family (system-actor RLS) follow-up should introduce a sentinel system UUID or nullable `actor_id`.
- [x] [Review][Defer] F121: `writeObservation` `toISOString().slice(0, 10)` throws RangeError for years > 9999. Theoretical; document the contract.
- [x] [Review][Defer] F122: Empty-fixtures mock-mode error path doesn't apply `applyDeadLetter` to the upload directly — it propagates to pg-boss → dead-letter handler → `markUploadFailed`. Multi-hop but eventually correct.

**Dismissed (~5):** Blind Low B15 (jsonb COALESCE on a NOT NULL column — paranoia code, harmless); Edge E-CR-23 (storage `data.type` undefined fallback — dead code per R1-P98, covered by R2-P116); Edge E-CR-15 (pending_review → processing arc) — folded into F119; Edge E-CR-25 (empty biomarkerName SELECT '' wasted query) — folded into R2-P122; Auditor F-R2-01 actor_id drift — folded into F120.

---

**Dismissed (R1, ~12):** B4 (RLS adversarial test — already F-item); A6 (aws-adapter eager import — fine, throws only on call); Edge cases on empty strings + leading zeros in `parseBrazilianDecimal` (intentional reject); scientific notation rejection (intentional); ISO datetime variants (already in F109); LOINC seed `Hemoglobina` substring overlap (no fuzzy lookup yet, no issue); E16 terminal-state lock-miss audit (folded into F114); E19 applyDeadLetter return ignored — re-classified as **patch R1-P106** (not dismiss); E27 seed duplicates (no current consumer breaks); E28 raw-SQL bypass — same as A2 / **R1-P94**; storage `data.type` empty — covered by R1-P98; mixed-batch silent drop — covered by R1-P100.

### Change Log

- 2026-05-22 — Code review round 2. **17 patches resolved (R2-P113 – R2-P129), 8 deferred (F115–F122), ~5 dismissed.** Three HIGH crash-recovery / transaction-boundary correctness bugs closed: **R2-P113** added a unique index on `extraction_review_queue(upload_id, biomarker_name, reason)` AND moved the terminal `applyUploadTransition` inside the `sql.begin()` block (dispatch + audit + terminal UPDATE now all-or-nothing); **R2-P114** explicit enumeration in `currentStatus()` — `processing` resumes, `pending_review|complete|failed` ack-skip, anything else throws to retry (the old fall-through swallowed crash-recovery cases); **R2-P115** dispatcher returns a `conflictCount` so the consumer no longer dead-letters an already-complete upload on idempotent retry. **Other Med fixes**: R2-P116 (`downloadStorageObject` drops dead `mimeType` from return), R2-P117 (accurate comment on numeric stringification — no precision claim), R2-P118 (short-circuit LOINC lookup for low-confidence — pure perf win), R2-P119 (raw confidence preserved when invalid — operator can tell 0 from garbage), R2-P120 (corrected boot warning copy describing the multi-hop dead-letter path), R2-P121 (per-field try/catch in dispatcher prevents single-LOINC-failure infinite-retry loop), R2-P122 (empty `biomarkerName` / undefined `valueText` silently skipped instead of NOT NULL violation), R2-P123 (storage perma-failure → direct `applyDeadLetter` with `reason=storage_unavailable`), R2-P124 (entry-time validation of `patientId`/`uploadId`/`storagePath`/`mimeType`), R2-P125 (`writeReviewQueueEntry` mirrors R1-P108's finite-numeric check), R2-P127 (helper comment reflects the new idempotency seam). **Low fixes**: R2-P126 (deleted unused `OBSERVATIONS_INSERT_SQL_SHAPE` constant), R2-P128 (spec hygiene — Clarification #7 + scope guardrails annotated `R1-P100 / R2-P128 SUPERSEDED`), R2-P129 (snapshot-sync test now imports BOTH worker's AND API's `UPLOAD_TRANSITIONS` and asserts `deepEqual` — the spec-promised drift guard is now actually wired). **150 unit tests green** (+2 since R1: split the optimistic-lock test into two cases per R2-P114 throw + pending_review skip; the existing test count carries forward through `conflictCount` schema change). Lint, typecheck, format all green. **Acceptance Auditor R2 verified all 20 R1 patches landed at claimed locations.**
